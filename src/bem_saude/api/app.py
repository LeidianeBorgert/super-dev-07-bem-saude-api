...
def criar_aplicacao() -> FastAPI:
    if configuracoes.eh_producao:
        logger.info("Iniciando aplicação em mode PRODUÇÃO(Swagger desabilitado)")
        app = FastAPI(
        ...
        logger.info("Iniciando aplicação em mode DESENVOLVIMENTO (Swagger habilitado)")
        app = FastAPI(
            title="Bem Saúde API",
            version="1.0.0",
            docs_url="/docs",
    ...
    @app.get("/health",tags=["Sistema"], summary="Health check", description="Verifica se a API está" \
    "respondendo")
    def health_check():
        return{
            "status":"ok",
